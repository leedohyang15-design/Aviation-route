...
켜져 있는 동안 조금씩 계속 조회해 **캐시에 저장**해두고, 비행기를 눌렀을 때 바로 쓴다.

- **경로 없는 기체도 화면에 그대로 보인다**(기본값). 실제로 경로가 없는 건 5% 정도라
  숨겨서 얻는 게 적고, 숨기려면 조회가 다 끝날 때까지 기다려야 했다.
  숨기고 싶으면 `filter.routeOnly` 를 켜면 된다.
...
- 궤도 등급 칩(저궤도/스타링크/중궤도/정지궤도)으로 원하는 것만 볼 수 있다.
  **스타링크는 기본 꺼져 있다** — 전체의 3분의 2라 켜두면 나머지가 안 보인다.
  칩을 누르면 군집이 한꺼번에 나타나 오히려 볼거리가 된다.
- 위성은 **점**으로, 선택한 하나만 **위성 아이콘**으로 그린다(수천 개를 아이콘으로 그리면
  서로 겹쳐 알아볼 수 없다). 위성엔 출발지·도착지가 없으므로 마커·지명·국기는 뜨지 않고,
  궤도는 지나온/남은 구분 없이 **한 줄**로 그린다.
