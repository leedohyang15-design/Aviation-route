...
켜져 있는 동안 **조금씩 계속 조회**해 캐시하고, 경로가 없다고 확인된 기체는 지도에서 숨긴다.

- 아직 조회 안 된 기체는 **일단 보인다.** 그래서 켠 직후엔 다 보이다가 서서히 정리된다.
  전체를 한 바퀴 도는 데 **첫날은 20~30분** 정도 걸린다.
- **캐시가 `Aviation Route.exe` 옆 `aviation-route-routes.json` 에 저장**되므로,
  **둘째 날부터는 켜자마자 대부분 정리된 상태**로 시작한다. (편명은 매일 같은 노선이라 재사용됨.
  이 파일은 지워도 되며, 지우면 다시 처음부터 조회한다.)
- 조회 API가 죽어도 기체가 사라지지 않는다(안전하게 "모름" 처리).
- 진행 상황은 `aviation-route.log` 의 `[routes]` 줄에서 확인한다
  (`route N / none N; cache N, N queued (≈N분 남음)` — `queued`가 줄어들면 정상).
- 조회 속도는 `.env` 로 조절: `ROUTE_LOOKUP_INTERVAL_MS`(기본 400 = 초당 2.5건.
  **낮출수록** 빨리 정리되지만 무료 API에 부담이 커진다).
