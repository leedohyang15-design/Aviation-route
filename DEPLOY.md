...
OpenSky는 출발지·도착지를 주지 않아서, 콜사인으로 `adsb.lol`에 물어봐야 경로를 알 수 있다.
켜져 있는 동안 **여러 대씩 묶어 미리 조회**해 캐시하고, 경로가 없다고 확인된 기체는 지도에서
숨긴다(컨트롤의 **"✓ 경로 있는 것만"** 토글로 끌 수 있다).

- 아직 조회 안 된 기체는 **일단 보인다.** 그래서 켠 직후엔 다 보이다가 몇 분에 걸쳐 정리된다.
- 조회 API가 죽어도 기체가 사라지지 않는다(안전하게 "모름" 처리).
- 진행 상황은 `aviation-route.log` 의 `[routes]` 줄에서 확인한다.
- 조회 속도는 `.env` 로 조절: `ROUTE_LOOKUP_CHUNKS_PER_POLL`(기본 10, 올리면 빨리 정리됨),
  `ROUTE_LOOKUP_CHUNK`(기본 100).
