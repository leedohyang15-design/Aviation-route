...
/**
 * One tile as a data URL. Colour scheme 0 (universal blue) and options `1_1`
 * — smoothed, with snow shown separately — are the free tier's defaults.
 */
async function fetchTile(
  ...
  const url = `${host}${path}/${WEATHER_TILE_PX}/${WEATHER_ZOOM}/${x}/${y}/0/1_1.png`
  try {
...
async function fetchFrame(
  ...
  for (let i = 0; i < wanted.length; i += BATCH) {
    ...
    const got = await Promise.all(
      wanted.slice(i, i + BATCH).map((t) => fetchTile(host, path, t.x, t.y))
    )
...
async function poll(onFrame: (f: WeatherFrame) => void): Promise<void> {
  ...
  for (const layer of LAYERS) {
    ...
    if (!newest) {
      opsLog(`[weather] index carries no ${layer} frames`)
      continue
    }
    ...
    if (!frame) {
      opsLog(`[weather] ${layer}: every tile failed — keeping the previous frame`)
      continue
    }
